...
6.商品详情
接口方法： GET
接口地址：product/detail?productId=6

'''
from send_method import *
api = 'product/detail?productId='#按需更改
file_path = './mallParameter/test_mallGoodsDetail.txt'#txt文件统一建在casefile下
...
if error != []:
    with open('./mallApiTest/error.txt',"a+",encoding="utf-8") as f:
        f.write('商品详情接口，出现“系统错误，请联系管理员”返回结果如下：'+"\n" + str(error) + "\n" + "===============" + "\n")  # 按需更改汉字
if L != []:
    with open('./mallApiTest/report.txt', 'a+', encoding="utf-8") as f:
        f.write('商品详情接口，成功运行%s次，成功与失败返回如下：'%ok+"\n"+str(L)+"\n"+"==============="+"\n")  # 按需更改汉字